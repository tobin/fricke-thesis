...
\caption[Optical spring transfer function (numerical and analytic)]{\label{fig:optical-spring}The optical spring effect:  As a cavity is detuned from resonance, the resonance of the differential mode deceases.  This figure compares the resultsof a numerical  (Optickle) model with analytic results. The units of the
  y-axis are $20\log_{10}$(displacement/force); the x-axis is Hz;
  color indicates cavity detuning in picometers.}
\end{figure}
The mechanical (force to position) transfer function of the differential mode of 
a detuned resonant cavity is shown in figure~\ref{fig:optical-spring}, both using
the analytic formulism developed above and using a numerical model implemented 
with Optickle.
