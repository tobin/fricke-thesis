...
test particles forming the end mirrors of the Michelson.  A suitably
polarized gravitational wave will modulate the difference between the
lengths of the two arms of the Michelson, producing a signal at its
output port.  The motivation for this particular configuration is that
variations in the laser frequency (i.e. noise) are equivalent to
\emph{common-mode} changes to the two arm lengths.  The Michelson
configuration allows us to sense gravitational waves and laser
frequency noise separately.

To study this configuration, we first derive the effect of the
Michelson on the incident laser field.  By inspection, we can write
down expressions for the transmission and reflection coefficients of
the Michelson.  We do this by following the paths that light can take
through the interferometer, keeping track of how the field amplitude
changes as it encounters each optic.  With a field of amplitude $E_0$
incident from the left (c.f. figure~\ref{fig:michelson}), a field with
amplitude $E_0/\sqrt{2}$ is reflected into the Y arm (assuming a
perfect beamsplitter), and a field with equal amplitude is transmitted
into the X arm.  The light in the Y arm acquires a phase $e^{i\phi_x}$
as it travels the length of the arm, then sees the amplitude
reflectivity $r_x\approx 1$ of the test mass, and acquires another
$\phi_x$ radians of phase as it travels down the arm on the way back
to the beamsplitter.  A fraction $1/\sqrt{2}$ of this light will be
transmitted through the beamsplitter, adding to the field that arrives
via the Y arm.  Likewise, a fraction $1/\sqrt{2}$ will be reflected by
the beamsplitter and travel back towards the laser source.  Putting
this story into equations, we arrive at the effective transmissivity
and reflectivity of the Michelson:
%
\begin{align}
t_M &\equiv \frac{E_{AS}}{E_0}   = \frac{1}{2}\left(r_x \exp \{i2\phi_x\} - r_y \exp\{i2\phi_y\} \right) \\
r_M &\equiv \frac{E_{REFL}}{E_0} = \frac{1}{2}\left(r_x \exp \{i2\phi_x\} + r_y \exp\{i2\phi_y\} \right)
\end{align}
where $r_{\{x,y\}}$ are the amplitude reflectivities of the x- and y-arm mirrors, and $\phi_{\{x,y\}}$ are 
the phases accumulated as the light travels from the beamsplitter to end mirrors.  

It is useful to change our variables to express these quantities in
terms of the differential and common reflectivies and phases.  Making
the substitutions
\begin{align}
...
and
\begin{align}
  \Delta r & = (r_x - r_y)/2      &  r_x   &= r + \Delta r \\
         r & = (r_x + r_y)/2      &  r_y   &= r - \Delta r
\end{align}
yields
\begin{align}
t_M  &= e^{i2\phi_+} \left( i r\sin \phi_- + (\Delta r) \cos \phi_- \right) 
\label{eq:michelson-transmission}\\
r_M  &= e^{i2\phi_+} \left( i r\cos \phi_- + (\Delta r) \sin \phi_- \right) 
\label{eq:michelson-reflection}
\end{align}

\begin{figure}
\includegraphics[width=\columnwidth]{figures/phasors.pdf}
\caption[Phasor diagram of DC readout]{\label{fig:mich-phasors}Phasor
  diagram of the fields at the output port due to the two arms of the
  Michelson.  Differential motion of the mirrors at the ends of the
  arms causes equal but opposite rotation of the two phasors, which
  modulates the amplitude of the resultant electric field at the
  output port ($E_{AS}$).}
\end{figure}
...
%
\begin{itemize}
\item Variations of the differential phase $\phi_-$ \emph{linearly couple} to the \emph{field amplitude} at the output port.  This can be seen via equation~\ref{eq:michelson-transmission} or the phasor diagram in figure~\ref{fig:mich-phasors}.
\item Variations of the common phase $\phi_+$ do not couple to the field at the output port, so long as we operate near $\phi_-=0$ and the contrast defect ($\Delta r$) is small.
\end{itemize}
%
...
anti-resonant in the cavity when the carrier is resonant.  The light
reflected from the cavity is directed onto a photodiode and an error
signal is produced by demodulating this photodiode signal.  The basic
setup is depicted in figure~\ref{fig:pdh-diagram}.
...
detuning (mismatch between the cavity length and laser frequency); the RF
...
is from resonance.  The PDH error signal is depicted in figure~\ref{fig:pdh-error}.
